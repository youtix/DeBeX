...
describe('DiamondTest', async function () {
  ...
  before(async function () {
    ...
    // ["0x01ffc9a7","0xf9f92be4","0xfe575a87","0x1a895266","0xad38bf22"]
    blacklistableFacet = await ethers.getContractAt('BlacklistableFacet', diamondAddress)
    // ["0x01ffc9a7", "0x8456cb59", "0x5c975abb", "0x3f4ba83a"]
  ...
  it('facets should have the right function selectors -- call to facetFunctionSelectors function', async () => {
    ...
    assert.sameMembers(result, selectors)
    selectors = getSelectors(blacklistableFacet)
  ...
  it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () => {
    ...
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0xfe575a87')
    )
    assert.equal(
      addresses[3],
      await diamondLoupeFacet.facetAddress('0xad38bf22')
    )
    assert.equal(
      addresses[4],
    ...
    assert.equal(
      addresses[4],
      await diamondLoupeFacet.facetAddress('0x3f4ba83a')
    )
  })
  ...
  it('add most functions and facets', async () => {
    ...
    const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(blacklistableFacet)
      },
      {
        facetAddress: addresses[4],
        action: FacetCutAction.Add,
        functionSelectors: getSelectors(pausableFacet)
      },
      {
    ...
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[3], facets)][1], getSelectors(blacklistableFacet))
    assert.sameMembers(facets[findAddressPositionInFacets(addresses[4], facets)][1], getSelectors(pausableFacet))
